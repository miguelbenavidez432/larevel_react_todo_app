...
export default function TButton({
  ...
  return (
    <>
      ...
      {to && (<Link to={to} className={classes.join(" ")} target={target} >{children}</Link>)}
      {!to && !href && (<button to={to} className={classes.join(" ")} target={target} onClick={onClick}>{children}</button>)}
    </>
  )
